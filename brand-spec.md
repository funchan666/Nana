...
## Album tab hit-area correction

The user clarified that the Photo album tab itself was opening content. Aspect-fill media previews were visually clipped without bounded interaction shapes, allowing oversized image hit regions to extend outside cards. Shared previews and profile media/room cards now explicitly constrain hit testing to their visible bounds. Decorative card backgrounds, video badges and the tab underline ignore hits. Tabs have independent 44pt rectangular targets and sit above the content layer. Photo album still switches the local tab state only; photo/video presentation remains confined to the corresponding thumbnail. This corrects the interaction boundary without changing the supplied visual design. Static source and whitespace checks only; no runtime verification under the project's no-run agreement.

## Search result layout correction

Search room rows now use a bounded 76×88 media thumbnail and separate title, host and status/category text. The full `NanaRoomArtwork` overlay component is no longer compressed into a narrow search thumbnail, which had forced status/title text into vertical wrapping and expanded row heights. Text has explicit line limits, and thumbnails retain bounded hit regions. Search input and the fixed-height category strip remain above the clipped, independently scrolling results.

People rows separate the profile action from the original Follow/Followed artwork instead of nesting buttons. Compact section headings show actual result counts; empty sections explain that no results match. Opening a result dismisses keyboard focus, and existing action notices remain visible. SwiftUI structure and supplied artwork are preserved; the previously evaluated Claude Design/Garden native guidance applies, with no new visual assets required. Source/diff checks passed; no build, tests, app launch or simulator verification.

## Search and filter visual redesign — 2026-09-23

The search surface uses a calm midnight base, 20pt horizontal spacing, rounded 24pt page typography, 20pt section headings, 14–16pt content titles and lilac/pink accents. People appear as individually actionable portrait cards (horizontal discovery rail, adaptive grid in People); rooms use compact photo-led rows, and posts use separate editorial text cards. Result counts remain derived from filtered catalog data. Empty states reuse the original search illustration. Input and category controls remain fixed above the clipped results, and profile/follow controls remain separate actions.

Two bespoke transparent raster illustrations were generated with the built-in host image tool under Garden's Host-Native workflow: `NanaSearchIllustration.imageset/NanaSearchIllustration.png` and `NanaFilterIllustration.imageset/NanaFilterIllustration.png`, inside `NanaHarbor/Assets.xcassets`. The images contain no baked text or external logos. Prompt record: `garden-gpt-image-2/prompt/nana-search-and-filters-20260923.md`. Both 1254×1254 RGBA files were visually inspected and their alpha channels checked. Existing supplied Follow, back, clear, plus/minus and blank action artwork remains intact.

Filters now use a scrolling custom native layout rather than Form, Stepper and menu pickers. All available catalog regions/languages are exposed as selection chips, with the currently selected value retained if the catalog changes. Adult age controls enforce 18 ≤ minimum ≤ maximum ≤ 70 and provide quick ranges. Edits live in a local draft; Apply commits the whole draft, while Cancel/swipe dismissal discards it. Reset restores the existing defaults (18–45, unrestricted region/language/gender) while preserving the search category. The Apply action remains outside the scroll area and controls have at least 44pt touch targets.

Claude Design's hierarchy/asset guidance and Garden's original raster workflow apply in the existing native SwiftUI stack. Taste and browser-only implementation/verification workflows were evaluated as outside this native task. Source/resource review and `git diff --check` passed. No compilation, tests, app launch, simulator or rendered screen verification was performed, in accordance with AGENTS.md.

## Reference-led conversation screen correction — 2026-09-23

The conversation card now retains the reference's complete structure for all recipients: a current avatar/name button, available level/age/region or creator label, three stat tiles, and a biography or mutual-follow explanation. The original glowing arrow (`041`) replaces the generated chevron. Follower/Following values are shown only when the profile supplies them; Friends reads a backward-compatible optional `friendsCount`. Missing values say Not shared rather than disappearing, inventing totals or displaying a dash. The current resolved profile name takes precedence over the conversation snapshot.

One composer represents the reference's empty/typing states, using the supplied More (`161`) and Send (`102`) artwork. The accessory strip starts expanded and uses the supplied Call (`047`) and Gift (`086`) artwork. Two original transparent raster icons were generated with the built-in image tool for Photo and Emoji, installed as `NanaChatPhotoIcon` and `NanaChatEmojiIcon` in the asset catalog. The existing original empty-conversation illustration is reused for a compact empty state and the quick-message control. Prompt record: `garden-gpt-image-2/prompt/nana-chat-icons-20260923.md`.

Quick messages and the emoji panel only insert draft text. Sending keeps the existing account, blocking and mutual-follow checks and retains the draft on unavailable delivery. The composer remains above the keyboard via a safe-area inset; focusing the field closes the quick-message/emoji panel and scrolls to the latest entry. Empty chats contain no fabricated messages. Photo delivery, private gifts and video calls still report unavailable; this visual correction does not pretend to add a messaging/call transport.

Claude Design's supplied-reference guidance and Garden's original raster workflow were used in SwiftUI; browser/Taste implementation scopes remain inapplicable. The new raster icons and existing source slices were visually inspected, resource wiring and `git diff --check` reviewed. No build, tests, app launch or simulator verification was performed, per AGENTS.md.

## Mutual-follow video preview and chat gift removal — 2026-09-23

The chat accessory bar now has Photo, Emoji and Call only. The private gift entry and its unavailable action are removed; room/store gifting is outside this chat request.

All three call entry points (conversation, public profile and friend list) share `nanaVideoCall(profile:)`. An account-scoped confirmed mutual follow is required before presenting or starting a camera session; public follow flags and local welcome samples cannot authorize calls. The shared gate renders a compact custom violet artwork card with a new original friendship/video illustration, the recipient identity, Follow/Got it and Not now. It never requests camera permission and it never follows anyone without an explicit button tap. Account changes, blocking and loss of mutual status close the waiting view.

The waiting view follows the supplied reference: full-bleed blurred recipient portrait, a centered portrait/name block and supplied camera-flip (`156`), hang-up (`165`) and camera (`162`) buttons. A real local preview sits at the upper right. There is no timeout, invented remote answer, fake call log or simulated media. The current service has no call signaling/RTC, so the screen explicitly says Local preview only and that the other person has not been called. Actual remote dialing/answering remains unimplemented until an authenticated calling transport exists.

`NanaLocalCallCamera` uses AVFoundation permission checking and an AVCaptureVideoPreviewLayer with portrait orientation and front-camera mirroring. Start/configure/flip/stop are serialized off the main thread. Permission completion is invalidated after cancellation, and preview stops on backgrounding, dismissal, interruption, camera-off and account/relationship revocation. Denied access offers Settings; missing cameras and runtime errors offer Retry. No microphone input, recording output or network sender is attached. Existing camera usage description already covers pre-call self-preview.

Native Design guidance and Garden Host-Native original asset production were used; Taste/browser implementation routes were evaluated as incompatible with this SwiftUI task. Palette: existing violet/electric lilac, white text and dark portrait treatment; type: 23 pt recipient/dialog titles, 14–17 pt body, 11–12 pt secondary labels; controls have at least 44 pt targets. Illustration: `NanaMutualCallIllustration.imageset/NanaMutualCallIllustration.png`; prompt: `garden-gpt-image-2/prompt/nana-mutual-video-call-20260923.md`. Original supplied artwork remains intact.

Source references: [Apple camera preview layer](https://developer.apple.com/documentation/AVFoundation/AVCaptureVideoPreviewLayer), [capture session setup](https://developer.apple.com/documentation/avfoundation/setting-up-a-capture-session). Validation is limited to source/resource inspection, Xcode project/Info.plist syntax validation and diff whitespace checks. No compilation, tests, app launch, camera access or simulator execution was performed, per project instructions.

## Camera/microphone permissions and unified media acquisition — 2026-09-23

Video-call preparation now requests both camera and microphone authorization through `NanaMediaPermissions`, after the mutual-follow gate. Both must be granted before the self-preview starts. The permission task is retained across the system permission dialog's temporary inactive state, avoiding duplicate requests; backgrounding or dismissal cancels subsequent work. Denial names the missing device permission and links to Settings. This still does not create a remote media transport or record audio in the local waiting room.

All existing media acquisition entry points now share a branded `NanaMediaSourcePicker`: registration/profile completion, both profile-editor avatar controls, personal album upload, live-cover selection, and chat attachments. Photo-only destinations offer Take photo and Photo library. Chat accepts photos and videos, adding Record video. The source chooser reuses the existing supplied camera/video/arrow assets and original photo icon/card surface, following the native Design/Garden guidance without creating web UI or additional stock artwork. Platform camera/library interfaces perform the actual acquisition.

Taking a photo requests camera only; recording a video requests camera and microphone. PHPicker provides scoped library selection without requiring full library permission. Captured and selected photos share size/orientation normalization; video files are copied out of the provider's temporary callback before it returns. Import can be cancelled, errors remain visible, unavailable cameras offer the library instead of silently changing sources. Limits are 25 MB for photo input, 250 MB for video, and 60 seconds for recording. A reference-counted local attachment owns and removes its copied file on removal/replacement/release. Account guards prevent returning selections from entering a different account's album, avatar, live cover or conversation.

Chat shows acquired media as a local, unsent attachment. The Send action still checks mutual follows and explicitly reports that the current service cannot send media; no fake message or upload success is created. Existing text drafts are preserved.

Camera and microphone usage descriptions are synchronized in AppInfo.plist and project.yml. Source references: [Apple picker media types](https://developer.apple.com/documentation/uikit/uiimagepickercontroller/mediatypes), [item-provider file representation](https://developer.apple.com/documentation/foundation/nsitemprovider/loadfilerepresentation(fortypeidentifier:completionhandler:)). Checks: source/resource inventory, Xcode project and Info.plist syntax, and git diff whitespace. No compile, tests, camera activation or simulator execution, per project instructions.

## Personal profile editor visual redesign — 2026-09-23

The editor now uses one compact personal identity card above four lightweight sections instead of repeating the bordered settings artwork around every group. A new original satin-violet backdrop with a restrained pearlescent star/ribbon decorates the identity card. The current account avatar and editable nickname form the focus, with Edit photo/Reset beside them. The artwork contains no baked text; the live nickname remains readable and updates as the user types.

The essentials use three compact rows on a quiet dark surface, with subtle separators and focus indication. Gender and interest choices are rounded tiles with clear selected color and the supplied check artwork (149). Interest columns adapt to width; the larger accessibility layout remains supported. The biography has a text area and a live 160-character count. Section headings use restrained indices and 19 pt rounded titles; key values are 16 pt, identity title 22 pt, supporting labels 11–13 pt. The footer uses a new original rectangular violet image button with a minimum 54 pt hit target. Focus scrolling and the keyboard-safe footer keep editing and saving reachable.

The account-scoped loading/save functions, shared photo/camera acquisition, avatar reset, birthday picker and all underlying field values are preserved. This change is scoped to NanaEditProfileView, not the settings cards used by other screens. No dummy profile information is introduced.

Design native hierarchy/reference guidance and Garden Host-Native image generation were applied; Taste and browser artifact routes were evaluated as incompatible with this native SwiftUI implementation. Original assets are `NanaProfileEditorHero.imageset/NanaProfileEditorHero.png` and `NanaProfileEditorButton.imageset/NanaProfileEditorButton.png`; final prompts are in `garden-gpt-image-2/prompt/nana-profile-editor-20260923.md`. Both rasters were visually inspected; originals were copied intact into the asset catalog. Validation: source/layout inspection and git diff whitespace checks only. No compilation, tests or simulator run, per project instructions.
